...
POPPLER_EXPORT int UTF16toUCS4(const Unicode *utf16, int utf16Len, Unicode **ucs4_out);
...
//   returns number of UCS-4 characters
POPPLER_EXPORT int TextStringToUCS4(const GooString *textStr, Unicode **ucs4);

// check if UCS-4 character is valid
POPPLER_EXPORT bool UnicodeIsValid(Unicode ucs4);

// is a unicode whitespace character
POPPLER_EXPORT bool UnicodeIsWhitespace(Unicode ucs4);
...
// REPLACEMENT_CHAR.
POPPLER_EXPORT int utf8CountUtf16CodeUnits(const char *utf8);
...
POPPLER_EXPORT int utf8ToUtf16(const char *utf8, uint16_t *utf16, int maxUtf16 = INT_MAX, int maxUtf8 = INT_MAX);

// Allocate utf16 string and convert utf8 into it.
POPPLER_EXPORT uint16_t *utf8ToUtf16(const char *utf8, int *len = nullptr);

// Count number of UTF-8 bytes required to convert a UTF-16 string to
// UTF-8 (excluding terminating NULL).
POPPLER_EXPORT int utf16CountUtf8Bytes(const uint16_t *utf16);
...
POPPLER_EXPORT int utf16ToUtf8(const uint16_t *utf16, char *utf8, int maxUtf8 = INT_MAX, int maxUtf16 = INT_MAX);

// Allocate utf8 string and convert utf16 into it.
POPPLER_EXPORT char *utf16ToUtf8(const uint16_t *utf16, int *len = nullptr);
...
POPPLER_EXPORT void unicodeToAscii7(const Unicode *in, int len, Unicode **ucs4_out, int *out_len, const int *in_idx, int **indices);
