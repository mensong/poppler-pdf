...
{
    const SplashColorMode mode = bitmap->getMode();
    ...
    if (!pipe->pattern && pipe->noTransparency && !state->blendFunc) {
        if (mode == splashModeMono1 && !pipe->destAlphaPtr) {
            pipe->run = &Splash::pipeRunSimpleMono1;
        } else if (mode == splashModeMono8 && pipe->destAlphaPtr) {
            pipe->run = &Splash::pipeRunSimpleMono8;
        } else if (mode == splashModeRGB8 && pipe->destAlphaPtr) {
            pipe->run = &Splash::pipeRunSimpleRGB8;
        } else if (mode == splashModeXBGR8 && pipe->destAlphaPtr) {
            pipe->run = &Splash::pipeRunSimpleXBGR8;
        } else if (mode == splashModeBGR8 && pipe->destAlphaPtr) {
            pipe->run = &Splash::pipeRunSimpleBGR8;
        } else if (mode == splashModeCMYK8 && pipe->destAlphaPtr) {
            pipe->run = &Splash::pipeRunSimpleCMYK8;
        } else if (mode == splashModeDeviceN8 && pipe->destAlphaPtr) {
            pipe->run = &Splash::pipeRunSimpleDeviceN8;
        }
    } else if (!pipe->pattern && !pipe->noTransparency && !state->softMask && pipe->usesShape && !(state->inNonIsolatedGroup && alpha0Bitmap->getAlphaPtr() != nullptr) && !state->blendFunc && !pipe->nonIsolatedGroup) {
        if (mode == splashModeMono1 && !pipe->destAlphaPtr) {
...
void Splash::pipeRun(SplashPipe *pipe)
{
    ...
    if (pipe->pattern) {
        ...
        if (mode == splashModeCMYK8 || mode == splashModeDeviceN8) {
    ...
    if (pipe->noTransparency && !state->blendFunc) {
        ...
        if (pipe->shape && state->blendFunc && pipe->knockout && alpha0Bitmap != nullptr) {
            destColorPtr = alpha0Bitmap->dataAt(0, alpha0Y + pipe->y);
            switch (mode) {
            case splashModeMono1:
        ...
        if (state->blendFunc) {
            if (mode == splashModeDeviceN8) {
                for (int k = 4; k < 4 + SPOT_NCOMPS; k++) {
                    cBlend[k] = 0;
                }
            }
            (*state->blendFunc)(cSrc, cDest, cBlend, mode);
        }
...
inline void Splash::pipeSetXY(SplashPipe *pipe, int x, int y)
{
    ...
    pipe->y = y;
    if (state->softMask) {
        pipe->softMaskPtr = state->softMask->dataAt(x, y);
    }
    switch (bitmap->getMode()) {
    case splashModeMono1:
        pipe->destColorPtr = bitmap->dataAt(x >> 3, y);
        pipe->destColorMask = 0x80 >> (x & 7);
        break;
    case splashModeMono8:
        pipe->destColorPtr = bitmap->dataAt(x, y);
        break;
    case splashModeRGB8:
    case splashModeBGR8:
        pipe->destColorPtr = bitmap->dataAt(3 * x, y);
        break;
    case splashModeXBGR8:
    case splashModeCMYK8:
        pipe->destColorPtr = bitmap->dataAt(4 * x, y);
        break;
    case splashModeDeviceN8:
        pipe->destColorPtr = bitmap->dataAt((SPOT_NCOMPS + 4) * x, y);
        break;
    }
    if (bitmap->getAlphaPtr() != nullptr) {
        pipe->destAlphaPtr = bitmap->alphaAt(x, y);
    } else {
        pipe->destAlphaPtr = nullptr;
    }
    if (state->inNonIsolatedGroup && alpha0Bitmap->getAlphaPtr() != nullptr) {
        pipe->alpha0Ptr = alpha0Bitmap->alphaAt(alpha0X + x, alpha0Y + y);
    } else {
...
inline void Splash::drawAAPixel(SplashPipe *pipe, int x, int y)
{
    ...
    if (x < 0 || x >= bitmap->getWidth() || y < state->clip->getYMinI() || y > state->clip->getYMaxI()) {
        return;
    }
...
Splash::Splash(SplashBitmap *bitmapA, bool vectorAntialiasA, SplashScreenParams *screenParams)
{
    ...
    inShading = false;
    state = new SplashState(bitmap->getWidth(), bitmap->getHeight(), vectorAntialias, screenParams);
    if (vectorAntialias) {
        aaBuf = new SplashBitmap(splashAASize * bitmap->getWidth(), splashAASize, 1, splashModeMono1, false);
...
Splash::Splash(SplashBitmap *bitmapA, bool vectorAntialiasA, SplashScreen *screenA)
{
    ...
    inShading = false;
    vectorAntialias = vectorAntialiasA;
    state = new SplashState(bitmap->getWidth(), bitmap->getHeight(), vectorAntialias, screenA);
    if (vectorAntialias) {
        aaBuf = new SplashBitmap(splashAASize * bitmap->getWidth(), splashAASize, 1, splashModeMono1, false);
        for (i = 0; i <= splashAASize * splashAASize; ++i) {
            aaGamma[i] = (unsigned char)splashRound(splashPow((SplashCoord)i / (SplashCoord)(splashAASize * splashAASize), splashAAGamma) * 255);
...
SplashError Splash::stroke(SplashPath *path)
{
    ...
    if (d1 > 0 && d1 * state->lineWidth * state->lineWidth < minLineWidth * minLineWidth) {
        ...
        strokeWide(path2, w);
    } else if (bitmap->getMode() == splashModeMono1) {
...
void Splash::fillGlyph2(int x0, int y0, SplashGlyphBitmap *glyph, bool noClip)
{
    ...
    if (xxLimit + xStart >= bitmap->getWidth())
        xxLimit = bitmap->getWidth() - xStart;
    if (yyLimit + yStart >= bitmap->getHeight())
        yyLimit = bitmap->getHeight() - yStart;
...
void Splash::arbitraryTransformMask(SplashImageMaskSource src, void *srcData, int srcWidth, int srcHeight, SplashCoord *mat, bool glyphMode)
{
    ...
    if (unlikely(scaledMask->getDataPtr() == nullptr)) {
    ...
    for (i = 0; i < nSections; ++i) {
        for (y = section[i].y0; y <= section[i].y1; ++y) {
            ...
            for (x = xa; x < xb; ++x) {
                ...
                pipe.shape = *scaledMask->dataAt(xx, yy);
...
void Splash::scaleMaskYdownXdown(SplashImageMaskSource src, void *srcData, int srcWidth, int srcHeight, int scaledWidth, int scaledHeight, SplashBitmap *dest)
{
    ...
    destPtr = dest->getDataPtr();
    if (unlikely(destPtr == nullptr)) {
        error(errInternal, -1, "dest->data is NULL in Splash::scaleMaskYdownXdown");
        return;
    }
...
void Splash::scaleMaskYupXdown(SplashImageMaskSource src, void *srcData, int srcWidth, int srcHeight, int scaledWidth, int scaledHeight, SplashBitmap *dest)
{
    unsigned char *lineBuf;
    unsigned int pix;
    int yp, yq, xp, xq, yt, y, yDest, yStep, xt, x, xStep, xx, d, d0, d1;
    int i;

    if (unlikely(dest->getDataPtr() == nullptr)) {
    ...
    // Start at row zero of dest bitmap
    yDest = 0;
    ...
    for (y = 0; y < srcHeight; ++y) {
        ...
        for (x = 0; x < scaledWidth; ++x) {
            ...
            // store one more more pixels
            for (i = yDest; i < yDest + yStep; ++i) {
                *dest->dataAt(x, i) = (unsigned char)pix;
            }
        }
...
void Splash::scaleMaskYupXup(SplashImageMaskSource src, void *srcData, int srcWidth, int srcHeight, int scaledWidth, int scaledHeight, SplashBitmap *dest)
{
    ...
    unsigned int pix;
    unsigned char *destPtr;
    int yp, yq, xp, xq, yt, y, yDest, yStep, yy, xd, xt, x, xDest, xStep;
    ...
    for (y = 0; y < srcHeight; ++y) {
        ...
        for (x = 0; x < srcWidth; ++x) {
            ...
            // store one or more pixels
            for (yy = yDest; yy < yDest + yStep; ++yy) {
                destPtr = dest->dataAt(xDest, yy);
                for (xd = 0; xd < xStep; ++xd) {
                    *destPtr++ = (unsigned char)pix;
                }
            }
    ...
    for (i = 0; i < nSections; ++i) {
        for (y = section[i].y0; y <= section[i].y1; ++y) {
            ...
            for (x = xa; x < xb; ++x) {
                ...
                if (srcAlpha) {
                    pipe.shape = *scaledImg->alphaAt(xx, yy);
                } else {
                    pipe.shape = 255;
                }
                if (vectorAntialias && clipRes2 != splashClipAllInside) {
...
void Splash::scaleImageYdownXdown(SplashImageSource src, void *srcData, SplashColorMode srcMode, int nComps, bool srcAlpha, int srcWidth, int srcHeight, int scaledWidth, int scaledHeight, SplashBitmap *dest)
{
    ...
    if (srcAlpha) {
        ...
        destAlphaPtr = dest->getAlphaPtr();
    } else {
        alphaLineBuf = nullptr;
        alphaPixBuf = nullptr;
        destAlphaPtr = nullptr;
    }
...
void Splash::scaleImageYupXdown(SplashImageSource src, void *srcData, SplashColorMode srcMode, int nComps, bool srcAlpha, int srcWidth, int srcHeight, int scaledWidth, int scaledHeight, SplashBitmap *dest)
{
    ...
    unsigned int alpha;
    unsigned char *destPtr;
    int yp, yq, xp, xq, yt, y, yDest, yStep, xt, x, xStep, xx, xxa, d, d0, d1;
    int i, j;
    ...
    for (y = 0; y < srcHeight; ++y) {
        ...
        for (x = 0; x < scaledWidth; ++x) {
            ...
            case splashModeMono8:
                for (i = 0; i < yStep; ++i) {
                    destPtr = dest->dataAt(x * nComps, yDest + i);
                    *destPtr++ = (unsigned char)pix[0];
                }
                break;
            case splashModeRGB8:
            ...
            if (srcAlpha) {
                ...
                alpha = (alpha * d) >> 23;
                for (i = 0; i < yStep; ++i) {
                    *dest->alphaAt(x, yDest + i) = (unsigned char)alpha;
                }
            }
        }
...
void Splash::scaleImageYupXup(SplashImageSource src, void *srcData, SplashColorMode srcMode, int nComps, bool srcAlpha, int srcWidth, int srcHeight, int scaledWidth, int scaledHeight, SplashBitmap *dest)
{
    ...
    unsigned int alpha;
    unsigned char *destPtr, *destAlphaPtr;
    int yp, yq, xp, xq, yt, y, yDest, yStep, xt, x, xDest, xStep;
    int i, j;

    // Scaled dimensions must match destination bitmap
    assert(dest->getHeight() == scaledHeight);
    assert(dest->getWidth() == scaledWidth);
    ...
    for (y = 0; y < srcHeight; ++y) {
        ...
        for (x = 0; x < srcWidth; ++x) {
            ...
            case splashModeMono8:
                for (i = 0; i < yStep; ++i) {
                    destPtr = dest->dataAt(xDest * nComps, yDest + i);
                    for (j = 0; j < xStep; ++j) {
                        *destPtr++ = (unsigned char)pix[0];
                    }
                }
            ...
            if (srcAlpha) {
                ...
                for (i = 0; i < yStep; ++i) {
                    destAlphaPtr = dest->alphaAt(xDest, yDest + i);
                    for (j = 0; j < xStep; ++j) {
                        *destAlphaPtr++ = (unsigned char)alpha;
                    }
                }
...
void Splash::scaleImageYupXupBilinear(SplashImageSource src, void *srcData, SplashColorMode srcMode, int nComps, bool srcAlpha, int srcWidth, int srcHeight, int scaledWidth, int scaledHeight, SplashBitmap *dest)
{
    ...
    for (int y = 0; y < scaledHeight; y++) {
        ...
        destAlphaPtr = srcAlpha ? dest->alphaAt(0, y) : nullptr;
        destPtr = dest->dataAt(0, y);
...
SplashError Splash::composite(SplashBitmap *src, int xSrc, int ySrc, int xDest, int yDest, int w, int h, bool noClip, bool nonIsolated, bool knockout, SplashCoord knockoutOpacity)
{
    ...
    if (src->getMode() != bitmap->getMode()) {
        return splashErrModeMismatch;
    }

    if (unlikely(bitmap->getDataPtr() == nullptr)) {
        return splashErrZeroImage;
    }
...
void Splash::compositeBackground(SplashColorConstPtr color)
{
    ...
    if (unlikely(bitmap->getAlphaPtr() == nullptr)) {
...
bool Splash::gouraudTriangleShadedFill(SplashGouraudColor *shading)
{
    ...
    if (!bDirectBlit) {
        ...
        if (unlikely(blitTarget->getDataPtr() == nullptr)) {
            error(errInternal, -1, "No memory for blitTarget in Splash::gouraudTriangleShadedFill");
            return false;
        }
        blitMaxPtr = blitTarget->dataAt(0, blitTarget->getHeight());
    ...
    if (shading->isParameterized()) {
        ...
        for (int i = 0; i < shading->getNTriangles(); ++i) {
            ...
            for (int Y = y[0]; Y <= y[2]; ++Y) {
                ...
                blitPtr = blitTarget->dataAt(scanLimitL * colorComps, Y);

                for (int X = scanLimitL; X <= scanLimitR && blitPtr + colorComps <= blitMaxPtr; ++X, colorinterp += scanColorMap0, blitPtr += colorComps) {
                    ...
                    assert(blitPtr == blitTarget->dataAt(colorComps * X, Y));

                    shading->getParameterizedColor(colorinterp, bitmapMode, blitPtr);
        ...
        for (int i = 0; i < shading->getNTriangles(); ++i) {
            ...
            for (int Y = y[0]; Y <= y[2]; ++Y) {
                ...
                for (int X = scanLimitL; X <= scanLimitR && blitPtr + colorComps <= blitMaxPtr; ++X, blitPtr += colorComps) {
                    // FIXME : standard rectangular clipping can be done for a
                    // complete scanline which is faster
                    // --> see SplashClip and its methods
                    if (!clip->test(X, Y))
    ...
    if (!bDirectBlit) {
        ...
        for (int X = 0; X < W; ++X) {
            for (int Y = 0; Y < H; ++Y) {
                if (!blitTarget->alphaAt(X, Y))
                    continue; // draw only parts of the shading!
                blitPtr = blitTarget->dataAt(colorComps * X, Y);
...
SplashError Splash::blitTransparent(SplashBitmap *src, int xSrc, int ySrc, int xDest, int yDest, int w, int h)
{
    ...
    case splashModeMono1:
        for (y = 0; y < height; ++y) {
            p = bitmap->dataAt(xDest >> 3, yDest + y);
            mask = 0x80 >> (xDest & 7);
            sp = src->dataAt(xSrc >> 3, ySrc + y);
            srcMask = 0x80 >> (xSrc & 7);
            for (x = 0; x < width; ++x) {
                if (*sp & srcMask) {
                    *p |= mask;
    ...
    case splashModeMono8:
        for (y = 0; y < height; ++y) {
            p = bitmap->dataAt(xDest, yDest + y);
            sp = src->dataAt(xSrc, ySrc + y);
            for (x = 0; x < width; ++x) {
                *p++ = *sp++;
            }
        }
    ...
    case splashModeBGR8:
        for (y = 0; y < height; ++y) {
            p = bitmap->dataAt(3 * xDest, yDest + y);
            sp = src->dataAt(3 * xSrc, ySrc + y);
            for (x = 0; x < width; ++x) {
                *p++ = *sp++;
                *p++ = *sp++;
                *p++ = *sp++;
    ...
    case splashModeXBGR8:
        for (y = 0; y < height; ++y) {
            p = bitmap->dataAt(4 * xDest, yDest + y);
            sp = src->dataAt(4 * xSrc, ySrc + y);
            for (x = 0; x < width; ++x) {
                *p++ = *sp++;
                *p++ = *sp++;
                *p++ = *sp++;
    ...
    case splashModeDeviceN8:
        for (y = 0; y < height; ++y) {
            p = bitmap->dataAt((SPOT_NCOMPS + 4) * xDest, yDest + y);
            sp = src->dataAt((SPOT_NCOMPS + 4) * xSrc, ySrc + y);
            for (x = 0; x < width; ++x) {
                for (int cp = 0; cp < SPOT_NCOMPS + 4; cp++)
                    *p++ = *sp++;
            }
    ...
    if (bitmap->getAlphaPtr() != nullptr) {
        for (y = 0; y < height; ++y) {
            q = bitmap->alphaAt(xDest, yDest + y);
            memset(q, 0x00, width);
        }
    }
